...
lines. The textarea stayed at 60px (rows=1: 18+18 padding + one 24px
line) even though CSS max-height is 200px and scrollHeight was ~228px.
Only the last lines showed, and the top visible line was sliced by
``.input-wrapper { overflow: hidden }``. Mobile already auto-resizes;
desktop ChatInput did not.
...
@pytest.mark.unit
def test_css_math_locks_one_row_at_60px_without_resize():
    """rows=1 used height is 18+18 padding + 24px line = 60px (the reported box)."""
    font_size = 16
    line_height = 1.5 * font_size
    padding_y = 18 + 18
    min_height = 56
    used = max(min_height, 1 * line_height + padding_y)
    assert used == 60
...
@pytest.mark.unit
def test_apply_resize_is_a_no_op_without_a_node():
    assert _eval_js("applyComposerResize(null, 200)") == 0
    assert _eval_js("applyComposerResize(undefined, 200)") == 0
