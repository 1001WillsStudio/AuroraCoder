...
is the same path as Continue: it posts the transcript the user already
has (``message`` null) on the existing conversation. The first send still
omits ``conversation_id`` so the gateway allocates it; the UI then keeps
the ``X-Conversation-ID`` from that response so Try Again can resend
into that same chat after a first-turn provider failure.
