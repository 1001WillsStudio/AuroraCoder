...
| `tests/test_tool_activity_display.py` | ✅ | Tool-activity cards must not dump `CODE_INTERPRETER` markers, LLM edit_file notes, or Python `cannot unpack NoneType` exceptions, and a failed `read_file` is `failed` (not green `complete`). Sanitizer + `convert_messages_for_frontend`; Node helpers in `toolActivityDisplay.js`; source scan of ToolActivity.jsx. |
