...
| `gateway.settings_store` | 🟡 | max_iterations range on update (`test_settings_max_iterations.py`); custom Base URL http(s) (`test_settings_custom_provider_url.py`); obfuscation round-trip still ⬜ |
...
and ``validateCustomProviderBaseUrl`` (typed Base URL must be http(s)).
`tests/test_settings_validation.py` covers the helpers. `tests/test_settings_max_iterations.py`
locks the store for max_iterations. `tests/test_settings_custom_provider_url.py`
locks the store: ``not-a-valid-url`` does not persist.
