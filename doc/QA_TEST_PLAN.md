...
| Try Again after provider 500 | ✅ | `tests/test_try_again_same_conversation.py` — Try Again resends the existing transcript like Continue (`message` null); first send still omits `conversation_id`; the UI keeps `X-Conversation-ID` so retry stays on one history item with one user line |
