...
| `tests/test_composer_autoresize.py` | ✅ | Desktop composer must grow with a multiline draft (rows=1 is ~60px; CSS max-height 200px). Helpers in `composerResize.js` run via Node; source scan locks ChatInput `useLayoutEffect` on `value`. |
...
| `tests/test_authenticated_workspace_fetch.py` | ✅ | Passworded ``/api/files`` is 401 without Bearer (middleware, same gate as chat). Desktop tree, viewer, and Discover go through ``api.js`` ``_headers()``; download/export fetch a blob (``<a href>`` cannot send Authorization). Node helper locks first-paint ``max_depth=5`` and one-level ``path`` + ``max_depth=1``. |
| `tests/test_delete_conversation.py` | ✅ | History delete removes a chat and nested subagent descendants. Store cascade + `DELETE` `deleted_ids`; desktop Node helpers decide whether the open chat should close. Mobile IIFE helper is executed for P→C→G (confirm count 2; open grandchild leaves to new chat). |
