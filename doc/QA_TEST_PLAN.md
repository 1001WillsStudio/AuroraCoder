...
| `tests/test_file_tree_keyboard.py` | ✅ | Workspace file/folder rows must be real buttons (Tab + Enter/Space), show a focus ring, and expose a keyboard path to Download / Export / Delete. Helpers in `frontend/src/utils/treeKeyboard.js` run via Node; source scan locks FileTree.jsx + file-tree.css. |
