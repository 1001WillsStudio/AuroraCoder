...
| `tests/test_settings_modal_focus.py` | ✅ | Settings overlay must be `role=dialog` `aria-modal=true`, move focus in from the gear, and trap Tab so + New Chat / Upload behind the dim cannot be reached. Helpers in `frontend/src/utils/focusTrap.js` run via Node; source scan locks SettingsPanel wiring. |
