...
| Try Again after provider 500 | ✅ | `tests/test_try_again_same_conversation.py` — error SSE carries `conversation_id`; retry with the same id does not fork a second history item or duplicate the user line; UI mints an id on first send via `newConversationId` (works when `crypto.randomUUID` is missing) |
