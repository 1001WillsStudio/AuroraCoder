...
Every function fails open: on any network/parse error it returns an inert
default (empty stance, empty recall results, a clear error string for
writes) rather than raising — a memory outage must never break the agent's
turn loop (design doc §18 "fail-open").
"""
...
def remember(
    ...
    """Write one typed memory. Returns {"ok": bool, "id"/"error": str}."""
    payload = {
        "content": content,
    ...
    try:
        resp = requests.post(
            f"{GATEWAY_URL}/api/memory/remember", json=payload, timeout=_TIMEOUT,
        )
        if resp.status_code >= 400:
            return {"ok": False, "error": f"gateway returned {resp.status_code}: {resp.text[:300]}"}
        return resp.json()
    except Exception as e:
        logger.warning("[memory_client] remember failed: %s", e)
